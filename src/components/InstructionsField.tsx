...
export default class InstructionsField extends React.Component<IProps> {
  ...
  fetchInstruction = () => {
    ...
    MakeGETCall('/api/instructions', { query: { instruction_id: this.props.instruction_id } }).then(res => {
      const detail = res[0].detail[this.props.instruction_detail_key]
      this.setState({
  ...
  updateInstruction = () => {
    MakePOSTCall('/api/instructions/' + this.props.instruction_id, {
      body: {
